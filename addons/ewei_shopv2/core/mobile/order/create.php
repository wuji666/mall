...
class Create_EweiShopV2Page extends MobileLoginPage
{
    ...
    public function integralcalculation()
    {
        global $_GPC;
        global $_W;
        $credit = pdo_fetch('select credit1 from ' . tablename('ewei_shop_member') . ' where id=:id limit 1', array(':id' => $_W['ewei_shopv2_member']['id']));
        if ($credit['credit1'] < 1) {
            exit(json_encode(array('code' => 1, 'msg' => '很抱歉！积分不足哦！')));
        }

        $marketprice = $_POST['createInfo']['coupon_goods'][0]['marketprice'];
        // 获取商品的数量
        $total = $_POST['total'];

        // 商品所用的积分价格
        $markprice2 = $total * $marketprice * 100;
        ...
        exit(json_encode(array('code' => 0, 'price' => $result)));
    ...
    {
        $data = $_POST;
        $productprice = $data['total']*$data['createInfo']['coupon_goods']['0']['marketprice'];
        setcookie("result", '', time() - 1);
        exit(json_encode(array('code'=>0,'price'=>$productprice)));
    }
    ...
    public function submit()
    {
        ...
        $order['shopid'] = $_W['ewei_shopv2_member']['shopid'];
