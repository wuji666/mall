...
class List_EweiShopV2Page extends WebPage
{
    protected function orderData($status, $st)
    {
        ...
        $psize = 20;
        if ($_W['user']['jobid'] == 6) {
            $store = pdo_get('ewei_shop_store', array('uid' => $_W['user']['uid']));
            $guide = pdo_getall('users', array('jobid' => 7, 'shopid' => $store['id']));
        }
        ...
        if ($_W['user']['jobid'] == 2) {
            $aa = pdo_getall('users', array('fid' => $_W['user']['uid'], 'jobid' => 6));
            foreach ($aa as $k => $v) {
                $a[$k] = pdo_get('ewei_shop_store', array('uid' => $v['uid']));
            }
            if ($a) {
                foreach ($a as $k => $v) {
                    $shop_id[$k] = $v['id'];
        ...
        if ($_W['user']['jobid'] == 6) {
            $aa = pdo_getall('ewei_shop_store', array('uid' => $_W['user']['uid']));
            if ($aa) {
                foreach ($aa as $k => $v) {
                    $shop_id[$k] = $v['id'];
                }
        ...
        if ($_W['user']['jobid'] == 5) {
            $aa = pdo_getall('ewei_shop_store', array('yid' => $_W['user']['uid']));
            if ($aa) {
                foreach ($aa as $k => $v) {
                    $shop_id[$k] = $v['id'];
                }
        ...
        if ($_W['user']['jobid'] == 2 && $aa != false) {
            $shop_id = implode(',', $shop_id);
            $statuscondition .= ' and o.shopid in ' . '(' . $shop_id . ')';
        }
        if ($_W['user']['jobid'] == 6 && $aa != false) {
            $shop_id = implode(',', $shop_id);
            $statuscondition .= ' and o.shopid in ' . '(' . $shop_id . ')';
        }
        if ($_W['user']['jobid'] == 5 && $aa != false) {
            $shop_id = implode(',', $shop_id);
            $statuscondition .= ' and o.shopid in ' . '(' . $shop_id . ')';
        }
        if ($_W['user']['jobid'] == 7 ) {
            $statuscondition .= ' and o.guideid = '.$_W['user']['uid'];
        }
