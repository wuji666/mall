...
class Index_EweiShopV2Page extends ComWebPage
{
    ...
    public function main()
    {
        ...
        } elseif ($_W['user']['jobid'] == 2) {
            $uid = pdo_getall('users', array('fid' => $_W['user']['uid']));
            foreach ($uid as $k => $v) {
                $uid2[$k] = $v['uid'];
            }
            if (isset($uid2)) {
    ...
    protected function post()
    {
        ...
        $lists = pdo_fetchall('SELECT * FROM ' . tablename('users') . 'Where jobid=6 and fid=:fid', array(':fid' => $_W['user']['uid']));
        $lists2 = pdo_fetchall('SELECT * FROM ' . tablename('users') . 'Where jobid=5 and fid=:fid', array(':fid' => $_W['user']['uid']));
    ...
    public function editintegral()
    {
        ...
        $integral = pdo_get('users',array('uid'=>$_GPC['id']),array('integral'));
        $res = pdo_update('users',$data,array('uid'=>$_GPC['id']));
		$integral2 = pdo_get('users',array('uid'=>$_GPC['id']),array('integral'));
        if ($integral['integral'] > $integral2['integral']){
            $type = 0;
        }else{
            $type = 1;
        }
        $record['integralnum'] = abs($integral['integral']-$integral2['integral']);
        $record['type'] = $type;
        $record['usetime'] = time();
        $record['mid'] = $_GPC['id'];
        pdo_insert('ewei_users_integral',$record);
		if ($res){
		   show_json(1,'修改成功');
        }else{
            show_json(0,'MMP！出了个未知的错误');
        }
