...
class Category_EweiShopV2Page extends MerchWebPage
{
		...
		if (!empty($_W['ispost'])) {
			...
			foreach ($_GPC['catname'] as $id => $catname) {
				...
				if (strpos($id, 'new_') !== false) {
					...
					mplog('goods.virtual.job.add', '添加分类 ID: ' . $insert_id);
				}
				else {
					pdo_update('ewei_shop_virtual_category', array('name' => $catname), array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
					mplog('goods.virtual.job.edit', '修改分类 ID: ' . $id);
				}
			}

			mplog('goods.virtual.job.edit', '批量修改分类');
			show_json(1, array('url' => merchUrl('goods/virtual/job')));
		}
	...
	public function delete()
	{
		...
		if (empty($item)) {
			$this->message('抱歉，分类不存在或是已经被删除！', merchUrl('goods/virtual/job', array('op' => 'display')), 'error');
		}

		pdo_delete('ewei_shop_virtual_category', array('id' => $id));
		mplog('goods.virtual.job.delete', '删除分类 ID: ' . $id . ' 标题: ' . $item['name'] . ' ');
		show_json(1, array('url' => merchUrl('goods/virtual/job', array('op' => 'display'))));
	}
}
