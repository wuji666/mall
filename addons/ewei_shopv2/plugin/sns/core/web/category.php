...
class Category_EweiShopV2Page extends PluginWebPage
{
	...
	public function displayorder()
	{
		...
		if (!empty($item)) {
			...
			plog('sns.job.edit', '修改分类排序 ID: ' . $item['id'] . ' 标题: ' . $item['name'] . ' 排序: ' . $displayorder . ' ');
		}
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('sns.job.edit', '修改版块分类 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_sns_category', $data);
				$id = pdo_insertid();
				plog('sns.job.add', '添加版块分类 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('sns/job', array('op' => 'display'))));
		}
	...
	public function delete()
	{
		...
		if (empty($item)) {
			message('抱歉，分类不存在或是已经被删除！', webUrl('sns/job', array('op' => 'display')), 'error');
		}

		pdo_delete('ewei_shop_sns_category', array('id' => $id));
		plog('sns.job.delete', '删除版块分类 ID: ' . $id . ' 标题: ' . $item['name'] . ' ');
		show_json(1);
	}
	...
	public function enabled()
	{
		...
		foreach ($items as $item) {
			...
			plog('sns.job.edit', '修改商品分类<br/>ID: ' . $item['id'] . '<br/>标题: ' . $item['name'] . '<br/>状态: ' . $_GPC['enabled'] == 1 ? '显示' : '隐藏');
		}
