...
class Category_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('creditshop.job.edit', '修改积分商城分类 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_creditshop_category', $data);
				$id = pdo_insertid();
				plog('creditshop.job.add', '添加积分商城分类 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('creditshop/job', array('op' => 'display'))));
		}
	...
	public function delete()
	{
		...
		if (empty($item)) {
			message('抱歉，分类不存在或是已经被删除！', webUrl('creditshop/job', array('op' => 'display')), 'error');
		}

		pdo_delete('ewei_shop_creditshop_category', array('id' => $id));
		plog('creditshop.job.delete', '删除积分商城分类 ID: ' . $id . ' 标题: ' . $item['name'] . ' ');
		show_json(1);
	}
	...
	public function enabled()
	{
		...
		foreach ($items as $item) {
			...
			plog('creditshop.job.edit', '修改商品分类<br/>ID: ' . $item['id'] . '<br/>标题: ' . $item['name'] . '<br/>状态: ' . $_GPC['enabled'] == 1 ? '显示' : '隐藏');
		}
