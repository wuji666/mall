...
class Index_EweiShopV2Page extends CashierWebPage 
{
	...
	public function goods() 
	{
		...
		if (!(empty($item['goods_ids']))) 
		{
			...
			$goods = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_purchase') . ' WHERE uniacid=:uniacid AND cashier=1 AND id IN (' . $goods_ids . ')', array(':uniacid' => $_W['uniacid']));
			if (!(empty($goods))) 
			{
				foreach ($goods as &$v ) 
				{
	...
	public function query_goods() 
	{
		...
		$res = pdo_fetchall('SELECT id,uniacid,title,subtitle,shorttitle,thumb FROM ' . tablename('ewei_shop_purchase') . ' WHERE uniacid=:uniacid AND merchid=:merchid AND cashier=1 ' . $where, $params);
		if (!(empty($res))) 
		{
			foreach ($res as &$v ) 
			{
