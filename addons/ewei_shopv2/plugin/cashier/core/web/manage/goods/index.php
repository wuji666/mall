...
class Index_EweiShopV2Page extends CashierWebPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$params = array('shopid'=>$_W['cashieruser']['storeid'],'auth_code' => $data['auth_code'], 'paytype' => $data['paytype'], 'openid' => (isset($userinfo['openid']) ? $userinfo['openid'] : ''), 'money' => $data['money'], 'deduction' => $data['deduction'], 'mobile' => $data['mobile'], 'operatorid' => (isset($_W['cashieruser']['operator']) ? $_W['cashieruser']['operator']['id'] : 0));
			$goods[0]['shopid'] = $_W['cashieruser']['storeid'];
	...
	public function get_goods() 
	{
		...
		{
				$where = '';
				$params = array(':uniacid' => $_W['uniacid'],':shop_id'=>$_W['cashieruser']['storeid']);
				if (!(empty($keyword))) 
				{
					if (is_numeric($keyword) && isset($keyword[7])) 
					{
				...
				$goods = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_purchase') . ' WHERE uniacid=:uniacid AND shop_id =:shop_id AND cashier=1  ' . $where, $params);
