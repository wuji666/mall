...
class CashierModel extends PluginModel
{
	...
	public function createOrder(array $array, $return = NULL, $can_sale = true)
	{
		...
		$array['shopid'] = isset($_W['cashieruser']['storeid']) ? $_W['cashieruser']['storeid'] : 0;
	...
	public function createGoodsOrder($goods, $openid = '')
	{
		...
		$order['isparent'] = 0;
		$order['transid'] = '';
		$order['shopid'] = $_W['cashieruser']['storeid'];
	...
	{
		global $_W;
		$order = $this->createOrder(array('shopid'=>$_W['cashieruser']['storeid'],'paytype' => $params['paytype'], 'openid' => isset($params['openid']) ? $params['openid'] : '', 'money' => (double) $params['money'], 'couponpay' => (double) $params['couponpay'], 'nosalemoney' => (double) $params['nosalemoney'], 'operatorid' => (int) $params['operatorid'], 'deduction' => (double) $params['deduction'], 'mobile' => (int) $params['mobile'], 'title' => $params['title']), 1, false);
	...
	public function setStocks($orderid = 0)
	{
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_purchase') . ' g on g.id=og.goodsid ' . ' where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		...
		foreach ($goods as $g) {
			...
			if (!empty($stocktype)) {
				...
				if (!empty($g['goodstotal']) && ($g['goodstotal'] != -1)) {
					...
					if ($totalstock != -1) {
						pdo_update('ewei_shop_purchase', array('total' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid'],'shopid'=>$_W['cashieruser']['storeid']));
					}
				}
			}
			...
			if (1 <= $order['status']) {
				if ($g['totalcnf'] != 1) {
					pdo_update('ewei_shop_purchase', array('sales' => $g['sales'] - $g['total']), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid'],'shopid'=>$_W['cashieruser']['storeid']));
				}
				...
				pdo_update('ewei_shop_purchase', array('salesreal' => $salesreal), array('id' => $g['goodsid'],'shopid'=>$_W['cashieruser']['storeid']));
			}
		}
	}
