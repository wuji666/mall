...
<div class="fui-cell-group">
    ...
    <?php  } ?>
    <div class="fui-cell">
        <div class="fui-cell-label" >积分抵用</div>
        <div class="fui-cell-info"></div>
        <div class="fui-cell-remark noremark">
            <input type="checkbox" name="cashier"  value="0" aria-invalid="false" class="valid" data-checked="false" >
        </div>
    </div>
    <script>
...
<div class="fui-navbar order-create-checkout">
    <a href="javascript:;" class="nav-item btn butt" style="background: #FFBF00;    width: 0.2%;">积分购买</a>
    <a href="javascript:;" class="nav-item total">
        <p style="color: #000">
            积分<span class="text-danger  bigprice"><span class="">
               <?php  echo $createInfo['integral']?></span></span>
        </p>
    </a>
    <a href="javascript:;" class="nav-item total">
        <p style="color: #000">
            金额 <span class="text-danger  bigprice"><span class="<?php  if(!$packageid && empty($exchangeOrder)) { ?>totalprice<?php  } ?>" id="price">
                <?php  echo price_format($realprice,2)?></span></span>
        </p>
    </a>
...
<script>
    $('.butt').click(function () {
        $.post("<?php  echo mobileUrl('order/create/buy')?>", {
            ...
            if (res.code==0){
                alert(res.msg);
                window.location.href ="http://app.ahlzzn.com/app/index.php?i=2&j=2&c=entry&m=ewei_shopv2&do=mobile&r=order&status=1"
            }else{
                alert(res.msg)
            }
        },'json')
