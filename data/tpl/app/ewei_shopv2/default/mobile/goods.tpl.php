...
<script type='text/html' id='tpl_goods_list'>
	...
	<div class="fui-goods-item" data-goodsid="<%g.id%>" data-type="<%g.type%>">
		<a <%if g.bargain>0%>href="<?php  echo mobileUrl('bargain/detail')?>&id=<%g.bargain%>"<%/if%><%if g.bargain<=0 && g.type!=9%>href="<?php  echo mobileUrl('goods/detail')?>&id=<%g.good_id%>"<%else%>href="<?php  echo mobileUrl('cycelbuy/goods/detail')?>&id=<%g.id%>" <%/if%>>
